...
class LinkedList:
  ...
  def addToHead(self,info): # O(1) Assigning a new head
    ...
    if self.size==0: #LL is empty
      ...
      n.next.previous = n # setting the previous of the next element to the new head
      n.previous = None
  ...
  def deleteTail(self):
    ...
    temp = self.tail.info # save the data in the tail to return after we delete it
